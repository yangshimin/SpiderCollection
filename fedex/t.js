...
let fedex_sensor_data = function(){
    ...
    win_.bmak = {
        ...
        cdoa: function (t) {
            ...
            console.log("执行deviceorientation事件的回调函数")
            try {
        ...
        cdma: function (t) {
            // devicemotion事件的回调函数
            console.log("执行devicemotion的事件回调函数")
            try {
        ...
        build_pua: function () {
            setTimeout(function(){
                bmak['pua'] = ['Mozilla/5.0 (', 'Windows;10.0.0;x86;64;)', ' AppleWebKit/537.36 (KHTML, like Gecko) Chrome/', '101.0.4951.41', ' Safari/537.36'].join("");
                console.log("执行了build_pua函数 pua", bmak['pua']);
...
fedex_sensor_data.prototype.first_get_sensor_data = function(){
    0 === bmak["doadma_en"] && (
        this.document["addEventListener"]("deviceorientation", bmak["cdoa"], !0),
            this.document["addEventListener"]("devicemotion", bmak["cdma"], !0),
            bmak["doadma_en"] = 1),
        bmak["doa_throttle"] = 0,
        bmak["dma_throttle"] = 0;
    bmak["to"]();
...
fedex_sensor_data.prototype.second_get_sensor_data = function(){
    this.document.dispatchEvent(event_ins.get_devicemotion());
    this.document.dispatchEvent(event_ins.get_deviceorientation());
    bmak["calc_fp"]()
    return bmak["sensor_data"]
}
